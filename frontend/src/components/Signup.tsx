...
const Signup: React.FC<SignupProps> = ({ onSwitchToLogin }) => {
  ...
  const handleSocialLogin = (provider: "google" | "facebook"): void => {
    if (provider === "google") {
      // Use the utility function to initiate Google OAuth
      initiateGoogleOAuth(API_BASE_URL);
    } else {
      console.log(`${provider} signup clicked`);
      // Facebook OAuth implementation can be added here later
      setNotification({
        type: "error",
        message: "Facebook signup is not implemented yet",
      });
    }
  };
              ...
              <div className="absolute inset-0 flex items-center justify-center p-4">
                <div className="flex items-center space-x-2 sm:space-x-4">
                  ...
                  <div className="bg-white bg-opacity-90 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg shadow-lg">
                    <span className="text-orange-600 font-bold text-4xl sm:text-4xl">
                      Habit Heat
                    </span>
                  </div>
                  <div className="w-8 h-10 sm:w-12 sm:h-16 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
